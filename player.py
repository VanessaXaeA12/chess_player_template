...
# import a hf dataset for model training
...
# 1. Prepare dataset
input_text = [
    {"text": f"{row['FEN']} {row['Next move']}"}
    for row in dataset
]

train_dataset = PrepDataset(tokenizer, input_text, max_length=128)
...
# 3. Train
trained_model = train_model(model, train_dataset, tokenizer)
...
class TransformerPlayer:
    def __init__(self, name):
        ...
        self.valid_token_ids = self.filter_to_valid_tokens()

    def filter_to_valid_tokens(self):
        valid = set("abcdefgh12345678")
        ids = []
        for tok in range(len(self.tokenizer)):
            if self.tokenizer.decode([tok]) in valid:
                ids.append(tok)
        return torch.tensor(ids)

    def get_move(self, fen):
        x = self.tokenizer.encode(fen, return_tensors="pt")

        # model returns logits directly
        logits = self.model(x)[:, -1, :]

        valid_logits = logits[:, self.valid_token_ids]
        probs = torch.softmax(valid_logits, dim=-1)

        sampled = torch.multinomial(probs, num_samples=4)
        chars = [self.tokenizer.decode([self.valid_token_ids[i]]) for i in sampled[0]]
